...
def parse_args():
    ...
    parser.add_argument('--save_json', type=Path, default=default_root / 'results_megadepth_eval.json',
                        help='Full results (per_bag + summary)')
    # Global summary だけを吐く JSON
    parser.add_argument('--save_summary_json', type=Path,
                        default=default_root / 'results_megadepth_eval_summary_jamma.json',
...
def evaluate_bag_with_tracks(
    ...
    if endpoints_0.shape[0] > 0:
        ...
        R_err_0N = float('nan')
        t_err_0N = float('nan')
        if endpoints_0.shape[0] >= bag_size:
