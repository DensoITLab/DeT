...
def get_resized_wh(w, h, resize=None):
    if resize is not None:  # resize the longer edge
        scale = resize / max(h, w) # 832/ 1280 = 0.65
...
def pad_bottom_right(inp, pad_size, ret_mask=False):
    ...
    if inp.ndim == 2:
        ...
        padded = np.zeros((inp.shape[0], pad_size, pad_size), dtype=inp.dtype) # 3, 832, 832
...
def read_megadepth_color(path, resize=None, df=None, padding=False):
    ...
    image = Normalize(image).numpy() # 3, w, h

    if padding:  # padding
        pad_to = max(h_new, w_new) # 832
    ...
    return image, scale, mask, torch.tensor([h_new, w_new]), (w,h), (w_new, h_new)
