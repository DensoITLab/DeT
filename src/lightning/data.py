...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        ...
        if dist.is_available() and dist.is_initialized():
            sampler = DistributedSampler(self.test_dataset, shuffle=False)
            logger.info(f"[rank:{self.rank}/{self.world_size}]: Using DistributedSampler for test dataset.")
        else:
            sampler = None
            logger.info(f"[rank:{self.rank}]: Using standard DataLoader (no DistributedSampler).")
