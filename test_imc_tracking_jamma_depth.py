...
def parse_args():
    ...
    parser.add_argument('--subset_dir', type=Path, default=default_root / scene / set_name / 'sub_set',
                        help='Contains Nbag_*.txt files (e.g., 3bag_015.txt)')
    ...
    parser.add_argument('--save_json', type=Path, default=default_root / 'results_imc_eval_jamma.json',
                        help='Full results (per_bag + summary)')
    # Global summary だけを吐く JSON
    parser.add_argument('--save_summary_json', type=Path,
                        default=default_root / 'results_imc_eval_summary_jamma.json',
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        #default=['jamma', 'jamma_legacy'], #'splg', 'jamma_legacy'],  # 必要に応じて eloftr 系もデフォルトに追加
        default=['jamma'], #'splg', 'jamma_legacy'],  # 必要に応じて eloftr 系もデフォルトに追加

        help=(
            'Which methods to run. '
            'Subset of {jamma, splg, jamma_legacy, eloftr, eloftr_legacy}. '
            'Example: --methods jamma eloftr'
        ),
    )
...
def run_splg_pair(device: torch.device,
                  ...
                  lg_model: Any = None):
    """
    Run SuperPoint + LightGlue on a single image pair and return:
      mk0, mk1, mconf, flops, runtime_ms, result_like_dict
    """
...
def evaluate_bag_with_tracks(
    ...
    for i in range(num_pairs):
        ...
        if prev_data:
            mean_dist = prev_result["diff_points_valid_mean_dist"]
    ...
    mean_dist_mean = float(np.mean(mean_dists)) 
...
def main():
    ...
    lg_model = LightGlue(features='superpoint',
                         depth_confidence=-1,
    ...
    all_methods_kwargs: Dict[str, dict] = {
        ...
        "siftlg": {"sp_model": sp_model, "lg_model": lg_model},  # SuperPoint なしで LightGlue 単体
    ...
    for method_name, results in all_results.items():
        ...
        # ---------- mean_dist_mean の global average ----------
        total_mean_dist = 0.0
        count_mean_dist = 0

        for r in results:
            if 'mean_dist_mean' in r:
                total_mean_dist += float(r['mean_dist_mean'])
                count_mean_dist += 1

        avg_mean_dist_mean = (
            total_mean_dist / count_mean_dist
            if count_mean_dist > 0 else 0.0
        )

        print(
            f"Global avg mean_dist_mean ({method_name}): "
            f"{avg_mean_dist_mean:.6f}"
        )
