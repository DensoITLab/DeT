...
The demo writes `tracks.json` and `comparison.png` under the output directory. The comparison figure shows NN-JamMa on top and DeT-JamMa on the bottom, using the same sampled start tracks from the shared 0-1 matches. It adds a zoom window after the third image for the common track with the largest NN/DeT displacement on that frame, highlighting the focused track points.
