...
using sequential JamMa matching on edges:
  0-1, 1-2, 2-3, 3-4
...
def build_jamma(
    ...
    model = PL_JamMa(config, pretrained_ckpt=jamma_ckpt, profiler=profiler)
...
def build_tracks_from_sequential_pairs(
    ...
    nn_link_mode: str = "greedy",   # "greedy" or "greedy_retry"
) -> Dict[int, dict]:
    ...
    for i, (mk0, mk1, mconf) in enumerate(pair_matches):
        ...
        if method == "nn":
            ...
            if nn_link_mode == "greedy":
            ...
            elif nn_link_mode == "greedy_retry":
                # 新方式:
                # 前トラックを高信頼順に処理し、使われていたら次近傍へ
                prev_scores = []
                for tid in prev_tids:
                    tr = tracks[int(tid)]
                    prev_scores.append(float(tr["confs"][-1]) if len(tr["confs"]) > 0 else 1.0)
                ...
                for ord_idx in prev_order:
                    prev_pt = prev_points[ord_idx]
                    tid = int(prev_tids[ord_idx])
                    ...
                    chosen_idx = -1
                    for k_idx in cand_order:
                        if d2[k_idx] > nn_max_d2:
                            break
                        if used_current[k_idx]:
                            continue
                        chosen_idx = int(k_idx)
                        break
                    ...
                    used_current[chosen_idx] = True
                    num_linked += 1

                    ptB = mk1[chosen_idx]
                    conf_val = float(mconf[chosen_idx]) if mconf is not None else 1.0
                ...
                raise ValueError(f"Unknown nn_link_mode: {nn_link_mode}")
            ...
            print(
                f"    [track nn/{nn_link_mode}] edge={i}->{i+1} "
                f"prev={len(prev_points)} cur={n_cur} "
                f"linked={num_linked} ended={num_ended} new={num_new}"
            )
...
def filter_full_length_tracks(tracks: Dict[int, dict], bag_size: int) -> Dict[int, dict]:
    ...
    for tid, tr in tracks.items():
        ...
        pts = tr["points"]
        if len(pts) > bag_size:
            print(f"  [warn] track {tid} has more points than bag_size: {len(pts)} > {bag_size}")
...
def process_scene(
    ...
    for bag_file in bag_files:
        ...
        for i in range(bag_size - 1):
            ...
            print(f"  [pair] {img_paths[i].name} - {img_paths[i+1].name}  matches={mk0.shape[0]}")

        track_method = "nn" if method == "nn-jamma" else "default"
        tracks = build_tracks_from_sequential_pairs(
            pair_matches=pair_matches,
            topk=topk,
            method=track_method,
            nn_max_d2=nn_max_d2,
            nn_link_mode=nn_link_mode,
        )
...
def parse_args():
    ...
    ap.add_argument("--nn_link_mode", type=str, default="greedy_retry", choices=["greedy", "greedy_retry"])
    ap.add_argument("--bag_size", type=int, default=5)
    ap.add_argument("--topk", type=int, default=200000)
    ap.add_argument("--nn_max_d2", type=float, default=1000000000000000)
    ...
    if args.dataset == "IMC":
        ...
        for scene_name in scenes:
            ...
            process_scene(
                ...
                topk=args.topk,
                nn_link_mode=args.nn_link_mode,
