...
def estimate_lo_relative_pose_px(x0_px, x1_px, K0, K1, pixel_thr, conf=0.99999):
    ...
    M, info = poselib.estimate_relative_pose(
        x0_px, x1_px,
        camera0.to_cameradict(), camera1.to_cameradict(),
        {"max_epipolar_error": float(pixel_thr), "success_prob": float(conf)}
    )
        ...
        if ransac_mode == "lo-ransac":
            ...
            if E is not None and inl is not None:
                ...
                if len(x0_in_n) >= 30:
                    ...
                    mask2 = mask2.reshape(-1).astype(bool) if mask2 is not None else np.ones((len(x0_in_n),), dtype=bool)
                    ...
                    if len(x0_in_px2) >= 30:
                        boot["success"] = True
                        boot["num_inliers"] = int(len(x0_in_px2))
...
def main():
    ...
    if args.method == "jamma_det":
    ...
    elif args.method in ("splg_sp", "splg_sift"):
        ...
        run_pair_fn = run_splg_pair
        run_pair_kwargs.update({"sp_model": sp_model, "lg_model": lg_model})
    ...
    for bf in bag_files:
        ...
        tracks = filter_full_length_tracks(tracks, args.bag_size)
...
def parse_args():
    ...
    ap.add_argument("--method", type=str, default="splg_sift", choices=["splg_sp", "splg_sift", "jamma_det", "nn-jamma"])
    ...
    ap.add_argument("--sp_max_keypoints", type=int, default=2048)
