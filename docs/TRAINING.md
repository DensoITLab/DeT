...
## Dataset setup
Training DeT/JamMa follows the same data organization as JamMa. Two data components are needed: the original datasets, such as ScanNet and MegaDepth, and the offline-generated dataset indices. The indices store scenes, image pairs, and metadata for training, validation, and testing. For MegaDepth, relative poses are cached directly in the index files. For ScanNet, relative poses are not stored because the resulting files would be very large.
...
- [MegaDepth undistorted images and processed depths](https://www.cs.cornell.edu/projects/megadepth/dataset/Megadepth_v1/MegaDepth_v1.tar.gz)
    ...
    - The downloaded data path is referred to as `/path/to/megadepth`.
- [D2-Net preprocessed images](https://drive.google.com/drive/folders/1hxpOsqOZefdrba_BqnW490XpNX_LgXPB)
    ...
    - The downloaded data path is referred to as `/path/to/megadepth_d2net`.
